...
static pthread_t g_uartThread = 0;
...
// TODO: 姿态检测进程        PoseDetectAiProcess            AI推理过程
// 不断调用这个函数
static HI_VOID PoseDetectAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (GetCfgBool("pose_detect_switch:support_pose_detect", true)) {
        
        // 第一次需要加载模型,
        if (g_workPlug.model == 0) {
            // 加载网络模型yolox， 并且包括了串口初始化
            ret = YoloxPoseDetectLoad(&g_workPlug.model);
            if (ret < 0) {
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(ret < 0, POSE_RELEASE,
                    "load pose detect model err, ret=%#x\n", ret);
            }
        }

        VIDEO_FRAME_INFO_S resizeFrm;

        // TODO: 这里应该进行填充的, 简化处理, 后续考虑
        // 将原始图像 1920 x 1080 --> 416x416 的大小        resize, 可能会存在拉伸

        // 432, 768, 等比例缩放
        ret = MppFrmResize(&frm, &resizeFrm, 768, 432);
        // ret = MppFrmResize(&frm, &resizeFrm, POSE_FRM_WIDTH, POSE_FRM_HEIGHT);

        // SAMPLE_PRT("Input weight:%d, height:%d\n", resizeFrm.stVFrame.u32Width, resizeFrm.stVFrame.u32Height);

        // 这里的图像是YUV格式的, cal时先转换成bgr的格式, 具体进行网络推理的部分

        // TODO: 
        // 0 用串口收到的模式   :
        ret = YoloxPoseDetectCal(g_workPlug.model, &resizeFrm, &frm, 0);


        // 推理失败就推出
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, POSE_RELEASE,
            "pose detect plug cal FAIL, ret=%#x\n", ret);
        
        // 传递给MPI, 在板端可视化输出图像
        ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
        
        SAMPLE_CHECK_EXPR_GOTO(ret != HI_SUCCESS, POSE_RELEASE,
            "HI_MPI_VO_SendFrame fail, Error(%#x)\n", ret);
...
// TODO: 跑的函数
static HI_VOID* uartRead(HI_VOID* arg)  // 200帧的速度
{
    // 不断获取串口信息
    while (HI_FALSE == g_bAiProcessStopSignal) {// 和那个的顺序一样

        usleep(20 * 1000);                               // 休眠1s， 控制通信帧率相同
        // SAMPLE_PRT("                                        uart fd  read   %d", uartFd_pose);
        // if(uartFd_pose != 0)
        read_uart_data(uartFd_pose, uartReadData);
        // SAMPLE_PRT(" >>>>>>>>>>>>>>  uart               %d  %d \n", uartReadData->mode, uartReadData->action1);
        // uartReadData->mode = 2;
        // uartReadData->action1 = 4;
    }
    return HI_NULL;
}
...
// TODO: 跑的函数
static HI_VOID* GetVpssChnFramePoseDetect(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 2000;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        // HandClassifyAiProcess(frm, voLayer, voChn);
        PoseDetectAiProcess(frm, voLayer, voChn);
    }
...
// TODO： 主要跑的线程 Thread 推理线程
static HI_S32 PoseDetectAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFramePoseDetect, NULL);
...
// TODO： 主要跑的线程 Thread 推理线程
static HI_S32 UartThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "UartRead") < 0) {
        HI_ASSERT(0);
    }
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_uartThread, NULL, uartRead, NULL);
...
static HI_S32 PauseDoUnloadPoseDetectModel(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    if (GetCfgBool("pose_detect_switch:support_pose_detect", true)) {
         ...
         */
        // s32Ret = Yolo2HandDetectResnetClassifyUnload(g_workPlug.model);
        s32Ret = YoloxPoseDetectUnload(g_workPlug.model);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload pose model err:%x\n", s32Ret);
        ConfBaseExt();
        g_num = 0;
    }
...
/*
 * 将sensor采集到数据显示到液晶屏上，同时创建线程运行姿态检测任务
 * 视频输入->视频处理子系统->视频输出->显示屏
 * VI->VPSS->VO->MIPI
 */
// TODO:0 姿态检测主函数
// 这个函数主要包括了2个函数，一个是推理线程，另一个是卸载模型
HI_S32 SAMPLE_MEDIA_POSE_DETECT(HI_VOID)
{
    // 返回值
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    ...
    // s32Ret = HandClassifyAiThreadProcess();
    SAMPLE_PRT(" >>>>>>>>>>>>>>>>>   pose detect ai thread process \n");
    s32Ret = PoseDetectAiThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));
    ...
    SAMPLE_PRT(" >>>>>>>>>>>>>>>>   uart thread process \n");
    s32Ret = UartThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "uart thread creat fail:%s\n", strerror(s32Ret));
    ...
    // 增加串口的线程
    pthread_join(g_uartThread ,NULL);
