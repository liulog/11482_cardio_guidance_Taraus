 ...
 * Superimpose one or more rectangular boxes in the frame
 */
int MppFrmDrawKeypoints(VIDEO_FRAME_INFO_S *frm, const Keypoints *kpts, uint32_t color, int thick)
{
    VGS_DRAW_LINE_S lines[VGS_MAX_LINE];
    int i;
    ...
    // 左
    int skeleton1[][2] = { {15, 13}, {13, 11}, {5, 11}, {5, 7}, {7, 9}};
    // 右
    int skeleton2[][2] = { {16, 14}, {14, 12}, {6, 12}, {6, 8}, {8,10}};

    // 其他
    int skeleton3[][2] = {  {11, 12}, {5, 6}, {1, 2}, {0, 1},{0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}};

    // for(int k=0;k<17;k++){
            // SAMPLE_PRT("                                                    keypoint x_y %d, %d\n",kpts->keypoints[2*k], kpts->keypoints[2*k+1]);
    // }

    // 绘制17个骨骼线
    for(int j=0;j<5;j++){
        // SAMPLE_PRT("                                                point index: %d, %d\n",2 * skeleton[j][0], 2 * skeleton[j][0] +1);
        // SAMPLE_PRT("                                                           : %d, %d\n",2 * skeleton[j][1], 2 * skeleton[j][1] + 1);
        // SAMPLE_PRT("                                                  draw line: %d, %d\n",kpts->keypoints[2 * skeleton[j][0]],kpts->keypoints[2 * skeleton[j][0] + 1]);
        // SAMPLE_PRT("                                                           : %d, %d\n",kpts->keypoints[2 * skeleton[j][1]],kpts->keypoints[2 * skeleton[j][1] + 1]);
        lines[j].stStartPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton1[j][0]]);
        lines[j].stStartPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton1[j][0] + 1]);

        lines[j].stEndPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton1[j][1]]);
        lines[j].stEndPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton1[j][1] + 1]);
        lines[j].u32Color = RGB888_YELLOW;
        lines[j].u32Thick = thick;
    }
    ...
    for(int j=0;j<5;j++){
        ...
        // SAMPLE_PRT("                                                  draw line: %d, %d\n",kpts->keypoints[2 * skeleton[j][0]],kpts->keypoints[2 * skeleton[j][0] + 1]);
        // SAMPLE_PRT("                                                           : %d, %d\n",kpts->keypoints[2 * skeleton[j][1]],kpts->keypoints[2 * skeleton[j][1] + 1]);
        lines[j+5].stStartPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton2[j][0]]);
        lines[j+5].stStartPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton2[j][0] + 1]);

        lines[j+5].stEndPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton2[j][1]]);
        lines[j+5].stEndPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton2[j][1] + 1]);
        lines[j+5].u32Color = RGB888_RED;
        lines[j+5].u32Thick = thick;
    }
    ...
    for(int j=0;j<9;j++){
        ...
        // SAMPLE_PRT("                                                  draw line: %d, %d\n",kpts->keypoints[2 * skeleton[j][0]],kpts->keypoints[2 * skeleton[j][0] + 1]);
        // SAMPLE_PRT("                                                           : %d, %d\n",kpts->keypoints[2 * skeleton[j][1]],kpts->keypoints[2 * skeleton[j][1] + 1]);
        lines[j+10].stStartPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton3[j][0]]);
        lines[j+10].stStartPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton3[j][0] + 1]);

        lines[j+10].stEndPoint.s32X = IntToOven(kpts->keypoints[2 * skeleton3[j][1]]);
        lines[j+10].stEndPoint.s32Y = IntToOven(kpts->keypoints[2 * skeleton3[j][1] + 1]);
        lines[j+10].u32Color = RGB888_WHITE;
        lines[j+10].u32Thick = thick;
    }

    return VgsDrawLines(frm, lines, 19);
}
