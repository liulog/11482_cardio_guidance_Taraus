...
int FrmToRgbImg(VIDEO_FRAME_INFO_S* srcFrm, IVE_DST_IMAGE_S *dstImg)
{
     ...
     */
    for (int i = 0; i < chnNum; i++) {                                          // FrmToOrigImg
    ...
    ret = IveImgCreate(dstImg, IVE_IMAGE_TYPE_U8C3_PLANAR, srcImg.u32Width, srcImg.u32Height);  // 创建dstImg
    SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != ret, ret, "Error(%#x), IveImgCreate failed!\n", ret);

    IVE_CSC_CTRL_S stCSCCtrl = { IVE_CSC_MODE_PIC_BT601_YUV2RGB};                               // YUV 转RGB
...
int ImgU3Crop(const IVE_IMAGE_S *src, IVE_IMAGE_S *dst, const RectBox* origBox)
{
    RectBox box = *origBox;
    ...
    int srcStrideY1 = src->au32Stride[1];
    int dstStrideY1 = dst->au32Stride[1];
    uint8_t *srcBufY1 = (uint8_t*)((uintptr_t)src->au64VirAddr[1]);
    uint8_t *dstBufY1 = (uint8_t*)((uintptr_t)dst->au64VirAddr[1]);
    uint8_t *srcPtrY1 = &srcBufY1[box.ymin * srcStrideY1];
    uint8_t *dstPtrY1 = dstBufY1;
    for (int h = 0; h < boxHeight; h++, srcPtrY1 += srcStrideY1, dstPtrY1 += dstStrideY1) {
        if (memcpy_s(dstPtrY1, boxWidth, srcPtrY1 + box.xmin, boxWidth) != EOK) {
            HI_ASSERT(0);
        }
    }
    HI_ASSERT(dstPtrY1 - dstBufY1 == boxHeight * dstStrideY1);

    int srcStrideY2 = src->au32Stride[2];
    int dstStrideY2 = dst->au32Stride[2];
    uint8_t *srcBufY2 = (uint8_t*)((uintptr_t)src->au64VirAddr[2]);
    uint8_t *dstBufY2 = (uint8_t*)((uintptr_t)dst->au64VirAddr[2]);
    uint8_t *srcPtrY2 = &srcBufY2[box.ymin * srcStrideY2];
    uint8_t *dstPtrY2 = dstBufY2;
    for (int h = 0; h < boxHeight; h++, srcPtrY2 += srcStrideY2, dstPtrY2 += dstStrideY2) {
        if (memcpy_s(dstPtrY2, boxWidth, srcPtrY2 + box.xmin, boxWidth) != EOK) {
            HI_ASSERT(0);
        }
    }
    HI_ASSERT(dstPtrY2 - dstBufY2 == boxHeight * dstStrideY2);
