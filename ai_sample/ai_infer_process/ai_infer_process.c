 ...
 * YoloX parameter
 */
static SAMPLE_SVP_NNIE_MODEL_S g_stYoloXModel = {0};
static SAMPLE_SVP_NNIE_PARAM_S g_stYoloXNnieParam = {0};
static SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S g_stYoloXSoftwareParam = {0};
    ...
    DetectObjInfo resBuf[], int resSize, int* resLen)
{
    ...
    *resLen = 0;
    ...
    u32RoiNumBias += ps32ClassRoiNum[0];    // 总个数
    for (i = 1; i < u32ClassNum; i++) {     // 每个类的个数
...
/*
 * 函数：YoloX software参数初始化
 * function : YoloX software param init
 */
// TODO: yolox software参数初始化
static HI_S32 SampleSvpNnieYoloXSoftwareInit(SAMPLE_SVP_NNIE_CFG_S* pstCfg, SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret;
    HI_U32 u32ClassNum = 0;
    HI_U32 u32BboxNum;
    ...
    // 416
    SAMPLE_PRT("u32OriImHeight: %d\n", pstSoftWareParam->u32OriImHeight);

    // 416
    pstSoftWareParam->u32OriImWidth = pstNnieParam->astSegData[0].astSrc[0].unShape.stWhc.u32Width;
    
    SAMPLE_PRT("u32OriImWidth: %d\n", pstSoftWareParam->u32OriImWidth);
    
    // 类别个数为 1
    pstSoftWareParam->u32ClassNum = 1;      
    
    // 三个输出尺寸对应的grid的尺寸
    // pstSoftWareParam->u32GridNumHeight = {52, 26, 13};
    // pstSoftWareParam->u32GridNumWidth = {52, 26, 13};

    // 后处理的阈值
    pstSoftWareParam->u32NmsThresh = (HI_U32)(0.3f*SAMPLE_SVP_NNIE_QUANT_BASE);     // nms 阈值
    pstSoftWareParam->u32ConfThresh = (HI_U32)(0.5f*SAMPLE_SVP_NNIE_QUANT_BASE);   // TODO: conf 阈值

    // 最多输出的个数
    pstSoftWareParam->u32MaxRoiNum = 5;
    ...
    u32BboxNum = 52*52 + 26*26 + 13*13;

    u32TmpBufTotalSize = SAMPLE_SVP_NNIE_YoloX_GetResultTmpBuf(pstSoftWareParam);

    // bboxnum个
    u32DstRoiSize = SAMPLE_SVP_NNIE_ALIGN16(u32ClassNum * u32BboxNum * sizeof(HI_U32) * (34 + 4));
    ...
    // 分配空间
    s32Ret = SAMPLE_COMM_SVP_MallocCached("SAMPLE_YOLOX_INIT", NULL, (HI_U64*)&u64PhyAddr,
        (void**)&pu8VirAddr, u32TotalSize);
    SAMPLE_SVP_CHECK_EXPR_RET(HI_SUCCESS != s32Ret, s32Ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,Malloc memory failed!\n");
    memset_s(pu8VirAddr, u32TotalSize, 0, u32TotalSize);
    ...
    /*
     * 设置每个tmp buffer地址， 前面的空间分配给了tmpbuf
     * Set each tmp buffer addr
     */
    pstSoftWareParam->stGetResultTmpBuf.u64PhyAddr = u64PhyAddr;
    pstSoftWareParam->stGetResultTmpBuf.u64VirAddr = (HI_U64)((HI_UL)pu8VirAddr);
    ...
    pstSoftWareParam->stDstRoi.u64PhyAddr = u64PhyAddr + u32TmpBufTotalSize;
    pstSoftWareParam->stDstRoi.u64VirAddr = (HI_U64)((HI_UL)pu8VirAddr + u32TmpBufTotalSize);
    pstSoftWareParam->stDstRoi.u32Stride = SAMPLE_SVP_NNIE_ALIGN16(u32ClassNum * u32BboxNum * sizeof(HI_U32) * ( 4 + 34 ));
    pstSoftWareParam->stDstRoi.u32Num = 1;
    pstSoftWareParam->stDstRoi.unShape.stWhc.u32Chn = 1;
    pstSoftWareParam->stDstRoi.unShape.stWhc.u32Height = 1;
    pstSoftWareParam->stDstRoi.unShape.stWhc.u32Width = u32ClassNum * u32BboxNum * ( 4 + 34 );
...
/*
 * 函数：YoloX software参数去初始化
 * function : YoloX software param Deinit
 */
static HI_S32 SampleSvpNnieYoloXSoftwareDeinit(SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    SAMPLE_SVP_CHECK_EXPR_RET(pstSoftWareParam == NULL, HI_INVALID_VALUE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error, pstSoftWareParam can't be NULL!\n");
...
/*
 * 函数：Yolox去初始化
 * function : Yolox Deinit
 */
static HI_S32 SampleSvpNnieYoloXDeinit(SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam,
    SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S* pstSoftWareParam, SAMPLE_SVP_NNIE_MODEL_S *pstNnieModel)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*
     * SAMPLE_SVP_NNIE_PARAM_S参数去初始化
    ...
    /*
     * SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S参数去初始化
     * Software deinit
     */
    if (pstSoftWareParam != NULL) {
        s32Ret = SampleSvpNnieYoloXSoftwareDeinit(pstSoftWareParam);
        SAMPLE_SVP_CHECK_EXPR_TRACE(HI_SUCCESS != s32Ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "Error,SampleSvpNnieYolov2SoftwareDeinit failed!\n");
    }
...
/*
 * 函数：YoloX参数初始化
 * function : YoloX Param init
 */
static HI_S32 SampleSvpNnieYoloXParamInit(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_YOLOX_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret;
    /*
     * 初始化hardware参数               这部分不动
     ...
     * Init software param
     */
    s32Ret = SampleSvpNnieYoloXSoftwareInit(pstCfg, pstNnieParam, pstSoftWareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, INIT_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),SAMPLE_SVP_NNIE_Yolox_SoftwareInit failed!\n", s32Ret);
    return s32Ret;
INIT_FAIL_0:
    s32Ret = SampleSvpNnieYoloXDeinit(pstNnieParam, pstSoftWareParam, NULL);
    SAMPLE_SVP_CHECK_EXPR_RET(HI_SUCCESS != s32Ret, s32Ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),SAMPLE_SVP_NNIE_Yolox_Deinit failed!\n", s32Ret);
    return HI_FAILURE;
}
...
/*
 * 函数：基于模型文件创建YoloX模型
 * function : Creat YoloX model basad mode file
 */
int YoloXCreate(SAMPLE_SVP_NNIE_CFG_S **model, const char* modelFile)
{
    SAMPLE_SVP_NNIE_CFG_S *self;
    HI_U32 u32PicNum = 1;
    HI_S32 s32Ret;
     ...
     * Load YoloX model
     */
    SAMPLE_SVP_TRACE_INFO("YoloX Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel((char*)modelFile, &g_stYoloXModel);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOX_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error, SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");

    /*
     * YoloX数初始化
     * YoloX软件参数设置在SampleSvpNnieYoloXSoftwareInit,
     * 如果用户更改了网络结构，请确保
     * SampleSvpNnieYoloXSoftwareInit函数中的参数设置是正确的
     */
    SAMPLE_SVP_TRACE_INFO("YoloX parameter initialization!\n");
    g_stYoloXNnieParam.pstModel = &g_stYoloXModel.stModel;
    s32Ret = SampleSvpNnieYoloXParamInit(self, &g_stYoloXNnieParam, &g_stYoloXSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOX_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SampleSvpNnieYolov2ParamInit failed!\n");
     ...
     */
    SAMPLE_PRT("model.base={ type=%x, frmNum=%u, chnNum=%u, w=%u, h=%u, stride=%u }\n",
        g_stYoloXNnieParam.astSegData[0].astSrc[0].enType,
        g_stYoloXNnieParam.astSegData[0].astSrc[0].u32Num,
        g_stYoloXNnieParam.astSegData[0].astSrc[0].unShape.stWhc.u32Chn,
        g_stYoloXNnieParam.astSegData[0].astSrc[0].unShape.stWhc.u32Width,
        g_stYoloXNnieParam.astSegData[0].astSrc[0].unShape.stWhc.u32Height,
        g_stYoloXNnieParam.astSegData[0].astSrc[0].u32Stride);

    // SAMPLE_PRT("model.soft={ class=%u, ori.w=%u, ori.h=%u, bnum=%u, \
        // grid.w=%u, grid.h=%u, nmsThresh=%u, confThresh=%u, u32MaxRoiNum=%u }\n",
    SAMPLE_PRT("model.soft={ class=%u, ori.w=%u, ori.h=%u, nmsThresh=%u, confThresh=%u, u32MaxRoiNum=%u }\n",
        g_stYoloXSoftwareParam.u32ClassNum,
        g_stYoloXSoftwareParam.u32OriImWidth,
        g_stYoloXSoftwareParam.u32OriImHeight,
        // g_stYoloXSoftwareParam.u32BboxNumEachGrid,
        // g_stYoloXSoftwareParam.u32GridNumWidth,
        // g_stYoloXSoftwareParam.u32GridNumHeight,
        g_stYoloXSoftwareParam.u32NmsThresh,
        g_stYoloXSoftwareParam.u32ConfThresh,
        g_stYoloXSoftwareParam.u32MaxRoiNum);

    *model = self;
    return 0;

    YOLOX_FAIL_0:
        SAMPLE_PRT("YoloXCreate SampleSvpNnieYoloXDeinit\n");
        SampleSvpNnieYoloXDeinit(&g_stYoloXNnieParam, &g_stYoloXSoftwareParam, &g_stYoloXModel);
        *model = NULL;
        return -1;
}
 ...
 * Destroy Yolov2 model
 */
void YoloXDestory(SAMPLE_SVP_NNIE_CFG_S *self)
{
    // printf("[YOLOX_DESTROY] ============ \n");
    SampleSvpNnieYoloXDeinit(&g_stYoloXNnieParam, &g_stYoloXSoftwareParam, &g_stYoloXModel);
    SAMPLE_COMM_SVP_CheckSysExit();
    free(self);
}
 ...
 */
// TODO: yolox中过去结果
static void YoloXFetchRes(SVP_BLOB_S *pstDstScore, SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, PoseObjInfo resBuf[], int resSize, int* resLen)
{   
    // SAMPLE_PRT("                YOLOX_FETCH_RESULT\n");
    ...
    *resLen = 0;
    ...
    // 目标的个数
    u32RoiNumBias = ps32ClassRoiNum[0];    // 总共有多少个目标

    // 结果个数
    // SAMPLE_PRT("ps32ClassRoiNum: %d\n", u32RoiNumBias);

    // SAMPLE_PRT("class num %d\n", u32ClassNum);
    // SAMPLE_PRT("roi num bias %d\n", u32RoiNumBias);
    // SAMPLE_PRT("class roi num %d\n", ps32ClassRoiNum[0]);

    // for (i = 0; i < u32ClassNum; i++) {             // 第一个类别
    i = 0;

        // SAMPLE_PRT(" ====== HELLO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

    
        // u32ScoreBias = u32RoiNumBias;               // 得分偏移
        u32ScoreBias = 0;

        // bbox偏移 再bbox后紧跟了points的内容，所以 4 + (17x2)
        // u32BboxBias = u32RoiNumBias * (4+17*2);
        u32BboxBias = 0;
        /*
         * 如果置信度分数大于结果阈值，则打印结果
         * If the confidence score greater than result threshold, the result will be printed
        ...
        j = 0;
        // 遍历所有目标
        for (j = 0; j < (HI_U32)u32RoiNumBias; j++) {
            // 得分 
            ...
            RectBox *box = &resBuf[resId].box;
            box->xmin = ps32Roi[u32BboxBias + j * (4 + 17*2) + 0];
            box->ymin = ps32Roi[u32BboxBias + j * (4 + 17*2) + 1];
            box->xmax = ps32Roi[u32BboxBias + j * (4 + 17*2) + 2];
            box->ymax = ps32Roi[u32BboxBias + j * (4 + 17*2) + 3];

            Keypoints *kpts = &resBuf[resId].keypoints;
            for(int k=0;k<17;k++){
                kpts->keypoints[2*k]=ps32Roi[u32BboxBias + j * (4 + 17*2) + 4 + 2 * k];
                kpts->keypoints[2*k+1]=ps32Roi[u32BboxBias + j * (4 + 17*2) + 4 + 2 * k + 1];
                // SAMPLE_PRT("                                one point: %d, %d  \n", kpts->keypoints[2*k],kpts->keypoints[2*k+1]);
            }

            if (box->xmin >= box->xmax || box->ymin >= box->ymax) {
                SAMPLE_PRT("yolo_orig: {%d, %d, %d, %d}, %f, discard for coord ERR\n",
                    box->xmin, box->ymin, box->xmax, box->ymax, f32Score);
            } else {
                resId++;
            }
 ...
 * Calculation yuv image
 */
int YoloXCalImg(SAMPLE_SVP_NNIE_CFG_S* self, const IVE_IMAGE_S *img, PoseObjInfo resBuf[], int resSize, int* resLen)
{
    // printf("[YOLOX_CAL_IMG]==============\n");
    SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S stInputDataIdx = {0};
    SAMPLE_SVP_NNIE_PROCESS_SEG_INDEX_S stProcSegIdx = {0};
    HI_S32 s32Ret;
    ...
    // 不确定它是否具有转换的功能
    // 使用img填充NNIE
    s32Ret = FillNnieByImg(self, &g_stYoloXNnieParam, 0, 0, img);               // 三维填充 bgr
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOX_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");
    ...
    stProcSegIdx.u32SegIdx = 0;
    // 使用NNIE进行前向推理
    s32Ret = SAMPLE_SVP_NNIE_Forward(&g_stYoloXNnieParam, &stInputDataIdx, &stProcSegIdx, HI_TRUE);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOX_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
    ...
    // 使用YOLOX获取结果, 上面forward会生成15个卷积的输出, 在get_result中有明显的体现
    s32Ret = SAMPLE_SVP_NNIE_YoloX_GetResult(&g_stYoloXNnieParam, &g_stYoloXSoftwareParam);
    s32Ret = HI_SUCCESS;


    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOX_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_YoloX_GetResult failed!\n");

    // yolox 解算结果
    YoloXFetchRes(&g_stYoloXSoftwareParam.stDstScore, &g_stYoloXSoftwareParam.stDstRoi, &g_stYoloXSoftwareParam.stClassRoiNum, resBuf, resSize, resLen);
