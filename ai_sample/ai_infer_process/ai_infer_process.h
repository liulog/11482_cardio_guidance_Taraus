 ...
 * Information of detected objects
 */
typedef struct PoseObjInfo {
    int cls; // The category of the object, an integer> 0
    RectBox box; // The rectangular area of the object (pixels)
    Keypoints keypoints; // 人体关键点的信息, 前34个表示关键点的offset, 后34个表示关键点的visual
    float score; // Object's credibility score
} PoseObjInfo;
 ...
 * Calculation yuv image
 */
int YoloXCalImg(SAMPLE_SVP_NNIE_CFG_S* self,
    const IVE_IMAGE_S *img, PoseObjInfo resBuf[], int resSize, int* resLen);
